...
function SchoolList(props) {
  ...
  const [school_search, setSchoolSearch] = useState("");
      ...
      if (st) {
        ...
        dispatch(getCities(data[0], (res, city_dt) => {
          if (res) {
            ...
            dispatch(getSchoolOrTeamList(city_dt[0], data[0], "SCHOOL", (res1, school_dt) => {
              if (res1) {
                debugger
                setLoading(false)
                setSchool(school_dt)
  ...
  useEffect(() => {
    if (school_search) {
      let input = school_search.toLowerCase();
      let resultStates = school_data.filter(i => i.name.toLowerCase().includes(input));
      setSchoolList(resultStates);
    }
    else {
      setSchoolList(school_data);
    }
        ...
        >
          ...
          <View style={{
            ...
            <TextInput
              ...
              onChangeText={(text) => setSchoolSearch(text)}
              value={school_search}
...

              {
                schoolList.map((st, index) => (
                  <TouchableOpacity key={`state-${index}`} style={{ marginTop: 15 }} onPress={() => {
                    Navigation.navigate("EditProfile", { fromRoute: 'school', school: st, selected_state: state_selected, city: city_selected })
                  }}>
